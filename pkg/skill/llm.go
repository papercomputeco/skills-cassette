...
// The LLM caller used for skill extraction. Self-contained: the cassette
// resolves provider credentials from its own configuration and environment,
// never from Tapes credential state.
...
// provider. The handler maps it to a clear 4xx rather than an opaque 500.
...
func NewLLMCaller(cfg LLMCallerConfig) (LLMCallFunc, error) {
	...
	if apiKey == "" && provider != providerOllama {
		...
		return nil, fmt.Errorf("%w: provider %q — set %s or CASSETTE_LLM_API_KEY", ErrNoAPIKey, provider, envVar)
	}
